...
describe('search', () => {
	...
	it('empty sections note is indexed without errors', async () => {
		...
		const db = getDb()
		const note = db
			.prepare("SELECT * FROM notes WHERE path LIKE '%empty-sections%'")
			.get()
		assert.ok(note)
	})
...
describe('tag filtering', () => {
	...
	it('include + exclude combined: pkm notes without method tag', async () => {
		...
		assert.ok(results.length > 0)
		assert.ok(
			results.every(r => r.tags.includes('pkm') && !r.tags.includes('method')),
		)
	})
})
...
describe('scope filtering', () => {
	it('array scope OR: results from multiple folders', async () => {
		...
		assert.ok(
			results.every(
				r => r.path.startsWith('notes/pkm/') || r.path.startsWith('notes/dev/'),
			),
		)
	})
	...
	it('scope with - exclusion: subfolder excluded', async () => {
		...
		assert.ok(
			results.every(
				r => r.path.startsWith('notes/') && !r.path.startsWith('notes/dev/'),
			),
		)
	})
})
...
describe('related mode', () => {
	...
	it('direction backlinks: no positive-depth results', async () => {
		...
		assert.ok(
			backlinks.length > 0,
			'zettelkasten should have backlinks from second-brain',
		)
	})
