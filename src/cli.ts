...
program
	...
	.option(
		'--mode <mode>',
		'Search mode: hybrid|semantic|fulltext|title',
		'hybrid',
	)
	.option(
		'--path <path>',
		'Note path for similarity/related search (overrides positional query)',
	)
	.option(
		'--scope <scope>',
	...
	.option(
		'--related',
		'Graph traversal: show notes linked to/from this note (path input only)',
	)
		...
		if (opts.related) {
			...
			for (const r of results) {
				...
				const context = r.snippet
					? truncateAtWord(
							r.snippet.replace(/\t/g, ' ').replace(/ {2,}/g, ' '),
							160,
						)
					: r.title
